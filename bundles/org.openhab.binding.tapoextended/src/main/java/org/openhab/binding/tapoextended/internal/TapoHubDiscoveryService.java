...
/**
 * Handler class for TAPO Hub Smart Home thing discovery
 *
 * @author Lucas de Mamann - Initial contribution
 */
@NonNullByDefault
public class TapoHubDiscoveryService extends AbstractDiscoveryService implements ThingHandlerService {
    private final Logger logger = LoggerFactory.getLogger(TapoHubDiscoveryService.class);
     ...
     * @param bridgeHandler
     */
    public TapoHubDiscoveryService() {
    ...
    public void activate() {
        TapoHubConfiguration config = bridge.getBridgeConfig();
    ...
    public void setThingHandler(@Nullable ThingHandler handler) {
        if (handler instanceof TapoHubHandler) {
            TapoHubHandler tapoBridge = (TapoHubHandler) handler;
    ...
    public DiscoveryResult createResult(JsonObject device) {
        ...
        properties.put(Thing.PROPERTY_SERIAL_NUMBER, device.get(DEVICE_PROPERTY_ID).getAsString());
    ...
    protected String getDeviceModel(JsonObject device) {
        try {
            String deviceModel = device.get(DEVICE_PROPERTY_MODEL).getAsString();
